        ...
        if (statsState is Resource.Loading) {
            ...
            LazyColumn(
                ...
                if (data.categoryStats.isEmpty()) {
                    item {
                        Text("No category data available.", color = Color.White.copy(alpha = 0.5f))
                    }
                } else {
            ...
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                Text(statsState.message ?: "An error occurred", color = ErrorRed, fontSize = 16.sp)
            }
        }
    }
}
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Box(modifier = Modifier.size(10.dp).background(PrimaryNeon, androidx.compose.foundation.shape.CircleShape))
                    Spacer(modifier = Modifier.width(4.dp))
                    Text("Income", color = Color.White, fontSize = 12.sp)
                }
                Spacer(modifier = Modifier.width(16.dp))
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Box(modifier = Modifier.size(10.dp).background(SecondaryNeon, androidx.compose.foundation.shape.CircleShape))
                    Spacer(modifier = Modifier.width(4.dp))
            ...
            Canvas(modifier = Modifier.fillMaxSize()) {
                ...
                monthlyStats.forEach { stat ->
                    ...
                    drawRoundRect(
                        color = SecondaryNeon,
                        topLeft = Offset(currentX + barWidth + 4.dp.toPx(), canvasHeight - expenseHeight),
    ...
    GlassCard(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            Row(
                ...
                Text(stat.categoryName, color = Color.White, fontWeight = FontWeight.SemiBold, fontSize = 16.sp)
                Text("$${String.format("%.2f", stat.totalAmount)}", color = Color.White, fontWeight = FontWeight.Bold, fontSize = 16.sp)
            }
            ...
            Spacer(modifier = Modifier.height(8.dp))
            Text("${String.format("%.1f", stat.percentage)}%", color = Color.White.copy(alpha = 0.5f), fontSize = 12.sp)
        }
    }
}
